...
router.post('/getNote', function(req,res,next){
    var get_date=req.body;
    var noteModel=global.dbHandle.getModel("note");
    noteModel.find(get_date,function(err, singleNote){
        if(err){
            return next(err);/*错误的话，把错误给运出来*/
        }else{
           res.json(singleNote);
...
router.post('/updateNote', function(req,res,next){
    ...
    var noteModel=global.dbHandle.getModel("note");
    noteModel.update({date: _date}, note, function(err){
        if(err){
            return next(err);/*错误的话，把错误给运出来*/
        }else{
            console.log("笔记已经更新成功了啊...");
            noteModel.find({},function(err,notes){
                if(err){
                    console.log("我也不知道怎么回事...明明已经更新了啊...");
                }else{
                    res.json(notes);
                }
            });
